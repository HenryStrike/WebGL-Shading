...
*/
  ...
  document.onkeydown = function (ev) { keydown(ev); }; // keydown camera
  ...
  var tick = function () {		    // locally (within main() only), define our 
    ...
    timeDelta = (current - lastTime) / 1000;
    lastTime = current;
...
function setCamera(canvas) {
  ...
  g_worldMat.lookAt(px, py, pz,	// center of projection
    tx, ty, tz,	// look-at point 
...
function keydown(ev) {
  ...
  var vt = new Vector3([px - tx, py - ty, pz - tz]);// direction
  vt.normalize();
  ...
  switch (ev.key) {
    case "w":
      px -= vt.elements[0] * timeDelta * speed;
      py -= vt.elements[1] * timeDelta * speed;
      pz -= vt.elements[2] * timeDelta * speed;
      tx -= vt.elements[0] * timeDelta * speed;
      ty -= vt.elements[1] * timeDelta * speed;
      tz -= vt.elements[2] * timeDelta * speed;
      break;
    case "a":
      var new_center = new Vector3([tx - px, ty - py, tz - pz]);
      var centerRotate = new Matrix4();
      centerRotate.rotate(rspeed, up.elements[0], up.elements[1], up.elements[2]); // rotate axis
      new_center = centerRotate.multiplyVector3(new_center);
      tx = px + new_center.elements[0];
      ty = py + new_center.elements[1];
      tz = pz + new_center.elements[2];
      break;
    case "s":
      px += vt.elements[0] * timeDelta * speed;
      py += vt.elements[1] * timeDelta * speed;
    ...
    case "Shift":
      var new_center = new Vector3([tx - px, ty - py, tz - pz]);
      var centerRotate = new Matrix4();
      centerRotate.rotate(rspeed, ut.elements[0], ut.elements[1], ut.elements[2]); // rotate axis
      new_center = centerRotate.multiplyVector3(new_center);
      tx = px + new_center.elements[0];
      ty = py + new_center.elements[1];
      tz = pz + new_center.elements[2];
    ...
    case "Control":
      ...
      break;
    case "q":
      px -= ut.elements[0] * timeDelta * speed;
      py -= ut.elements[1] * timeDelta * speed;
      pz -= ut.elements[2] * timeDelta * speed;
      tx -= ut.elements[0] * timeDelta * speed;
      ty -= ut.elements[1] * timeDelta * speed;
      tz -= ut.elements[2] * timeDelta * speed;
      break;
    case "e":
      px += ut.elements[0] * timeDelta * speed;
      py += ut.elements[1] * timeDelta * speed;
